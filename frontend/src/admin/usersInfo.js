...
const Users = () => {
  ...
  const handleDeleteUser = (userId) => {
    ...
    if (window.confirm('Are you sure you want to delete this user?')) {
      const url = `http://localhost/Reactjs/react-PlusPackage/backend/admin/DeleteUser`;
  ...
  const handleEditClick = (userId) => {
    fetch(`http://localhost/Reactjs/react-PlusPackage/backend/admin/EditFormData?userId=${userId}`)
      ...
      .then(data => {
        if (data.success) {
          navigate('/EditUser', { state: { user: data.user } });
        } else {
          ...
          <div className="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-3">
            ...
            <button className="btn btn-success shadow-sm d-flex align-items-center gap-2 px-4 py-2">
              <Link to="/AddUser" className="text-decoration-none text-light">
                <FontAwesomeIcon icon={faUserPlus} size="lg" /> Add User
              </Link>
            </button>
          </div>
